...
public function store(Request $request){
    $this->validate($request,[
        ...
        'kursi'=>'required',
        'meja'=>'required',
        'gambar_kelas'=>'required|image|mimes:jpeg,png,gif|max:2048',//Menambahkan validasi file gambar yang bisa di upload
    ]);
    ...
    Kelas::create([
        ...
        'gambar_kelas' =>$gambar_kelas_name,//simpan nama teks file gambar dalam database
...
public function update(Request $request, $id_kelas){
    ...
    $this ->validate($request,[
        ...
        'kursi'=>'required',
        'meja'=>'required',
        'gambar_kelas'=>'required|image|mimes:jpeg,png,gif|max:2048',//validasi optional untuk gambar
    ]);

if ($request->hash_file('gambar_kelas')){
    $gambar_kelas=$request->file('gambar_kelas');
    $gambar_kelas_name=time().'.'.$gambar_kelas->getClientOriginalExtension();
    $gambar_kelas->move(public_path('upload_gambar'), $gambar_kelas_name);
...
//Perbarui data lainnya sesuai dengan kebutuhan 
$kelas->save();
return redirect("/kelas");
}
